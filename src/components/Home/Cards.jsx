...
const Cards = ({ home, setAddToDo, data, refreshTasks, setUpdatedData }) => {
  ...
  const handleCompleteTask = async (id) => {
    try {
      await axios.put(`http://localhost:8080/auth/update-complete-tasks/${id}`, {}, { headers });
  ...
  const handleImportant = async (id) => {
    try {
      await axios.put(`http://localhost:8080/auth/update-imp-tasks/${id}`, {}, { headers });
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:8080/auth/delete-task/${id}`, { headers });
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
        {data &&
          data.map((items) => (
            <div
              key={items._id}
              className="flex flex-col justify-between bg-white rounded-lg p-5 shadow hover:scale-[1.02] transition-transform duration-300"
            >
              <div>
                <h3 className="font-semibold text-xl text-[#003366]">{items.title}</h3>
                ...
                <div className="text-sm text-gray-600 mt-2 space-y-1">
                  ...
                  {home === "false" && items.userId && (
                    <p><strong>Shared By:</strong> {items.userId.name}</p>
                  )}
              ...
              <div className="mt-4 flex flex-col sm:flex-row gap-3 w-full">
                <button
                  ...
                  } text-white px-3 py-2 rounded flex-1 font-semibold hover:shadow-md transition`}
                  onClick={() => handleCompleteTask(items._id)}
                >
                  {items.complete ? "Completed" : "In Complete"}
                </button>

                <div className="flex items-center justify-around flex-1 gap-4 text-xl text-gray-700">
                  ...
                  <button onClick={() => { setShareTaskId(items._id); setShareToDo(true); }} className="hover:text-[#003366]">
                    <FaShareAlt />
                  </button>

                  <button onClick={() => handleDelete(items._id)} className="hover:text-[#003366]">
                    <MdDelete />
                  </button>
                </div>
              </div>
        ...
        {home === "true" && (
          <button
            className="flex flex-col items-center justify-center bg-white p-6 rounded-lg shadow hover:scale-[1.02] transition-transform duration-300"
            onClick={() => setAddToDo("fixed")}
          >
            <IoAddCircleSharp className="text-5xl text-[#003366]" />
            <span className="mt-2 text-xl font-semibold text-[#003366]">Add Task</span>
          </button>
        )}
      </div>
        ...
        <>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50 px-4">
            <div className="w-full max-w-md bg-white p-6 rounded-lg shadow-lg">
              <h1 className="text-[#003366] text-3xl font-semibold text-center mb-5">Share Task</h1>
              <input
                type="email"
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  onClick={() => { setShareToDo(false); setShareEmail(""); }}
                >
                  Cancel
                </button>
              </div>
